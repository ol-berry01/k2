<?php
$k2_enable_section = get_theme_mod( 'k2_blog_enable', true );
if ( $k2_enable_section || is_customize_preview() ) :
?>
<div id="blog-section" class="blog-section" <?php if( false == $k2_enable_section ): echo 'style="display: none;"'; endif ?>>
    <h3 class="section-title wow fadeIn"><?php echo esc_html( get_theme_mod( 'k2_blog_title', __( 'From The Blog', 'k2' ) ) ); ?></h3>
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class('blog-item wow fadeIn'); ?>>
				...
				<div class="blog-item-image">
					<a href="<?php echo esc_url( get_permalink() ) ?>" class="ql_thumbnail_hover" rel="bookmark" title="Permanent Link to <?php the_title_attribute(); ?>">
						<?php the_post_thumbnail( 'k2_blog-section' ); ?>
					</a>
				</div><!-- /post_image -->
				<?php } ?>
                ...
                <div class="blog-item-entry">
                	...
                	<?php the_excerpt(); ?>
                    <a href="<?php echo esc_url( get_permalink() ) ?>" class="read-more"><?php esc_html_e( 'Read More', 'k2' ); ?> <i class="fa fa-angle-right"></i></a>
                </div>
            </article>
